...
def gen_peaklist_weak(nuclei_frequencies : list[float] | list[int], J_couplings : np.ndarray) -> PeakList:
    ...
    for i, nuclei in enumerate(nuclei_frequencies):
        ...
        peak_list += signal
    return _reduce_peaks(sorted(peak_list))
...
def _multiplet(signal : tuple, couplings : Generator[tuple[ArrayLike, int]]) -> PeakList:
    ...
    -----
        Adapted from nmrsim's firstorder.py.
    """
    peak_list : PeakList = [signal]
    for coupling in couplings:
        for _ in range(coupling[1]):
            peak_list = _doublet(peak_list, coupling[0])
    return sorted(_reduce_peaks(peak_list))

def _doublet(peak_list, coupling_constant) -> PeakList:
    """
    ...
    for freq, intensity in peak_list:
        new_peak_list.append((freq - coupling_constant / 2, intensity / 2))
        new_peak_list.append((freq + coupling_constant / 2, intensity / 2))
    return new_peak_list

def _reduce_peaks(unsorted_peak_list, tolerance=0) -> PeakList:
    """
    ...
    Returns
    -------
        PeakList: A new list of peaks where adjacent peaks within the specified tolerance
            have been combined.
    ...
    peak_list : PeakList = sorted(unsorted_peak_list) 
    ...
    for peak in peak_list:
        ...
        if peak[0] - work[-1][0] <= tolerance:
            ...
            new_peak_list.append(peak_sum(work))
            work = [peak]
...
def peak_sum(peak_list : PeakList) -> Peak:
    """
    ...
    -----
        Adapted from nmrsim's math.py
    """
    frequency_total : float | Literal[0] = sum(peak[0] for peak in peak_list)
    intensity_total : float | Literal[0] = sum(peak[1] for peak in peak_list)

    return (frequency_total / len(peak_list), intensity_total)
