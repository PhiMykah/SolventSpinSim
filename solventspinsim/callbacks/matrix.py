        ...
        for i, row in enumerate(np.array(spin.couplings)):
            for j, col in enumerate(row):
                if dpg.does_item_exist(f"coupling_{i}_{j}"):
                    dpg.configure_item(
                        f"coupling_{i}_{j}", min_value=min_value, max_value=max_value
                    )
        return
        ...
        for i, row in enumerate(np.array(spin.couplings)):
            with dpg.table_row():
                ...
                for j, col in enumerate(row):
                    ...
                    if i != j:
                        dpg.add_drag_float(
                            label=f"##coupling_{i}_{j}",
                            tag=f"coupling_{i}_{j}",
                            width=-1,
                            min_value=min_value,
                            max_value=max_value,
                        ...
                        dpg.add_text(
                            "", label=f"##coupling_{i}_{j}", tag=f"coupling_{i}_{j}"
                        )
