...
class SimulationSettings(Graphic):
    ...
    def render(self) -> None:
        ...
        dpg.add_text(
            default_value="Simulation Settings",
            tag="sim_settings_title",
            parent=self.parent,
        )

        field_strength, points, intensity, half_height_width, use_settings = (
            self.params.keys()
        )
        with dpg.value_registry():
            dpg.add_float_value(
                default_value=self.params[field_strength], tag=f"{field_strength}_value"
            )
            dpg.add_int_value(default_value=self.params[points], tag=f"{points}_value")
            dpg.add_float_value(
                default_value=self.params[intensity], tag=f"{intensity}_value"
            )
            dpg.add_float_value(
                default_value=self.params[half_height_width],
                tag=f"{half_height_width}_value",
            )
            dpg.add_bool_value(
                default_value=self.params[use_settings], tag=f"{use_settings}_value"
            )
        ...
        with dpg.table(header_row=False, parent=self.parent):
            ...
            with dpg.table_row():
                dpg.add_checkbox(
                    ...
                    callback=lambda sender, value, ui: setattr(
                        ui, "use_settings", value
                    ),
                    user_data=self.ui,
