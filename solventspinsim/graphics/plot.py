...
class PlotWindow(Graphic):
    ...
    def render(self) -> None:
        ...
        with dpg.plot(
            ...
            tag=main_plot,
            width=-1,
            height=self.params[main_plot],
        ):
            dpg.add_plot_legend()
            # REQUIRED FOR PLOTTING: create x and y axes
            dpg.add_plot_axis(
                dpg.mvXAxis, label=self.params[main_x_axis], tag=main_x_axis
            )
            dpg.add_plot_axis(
                dpg.mvYAxis, label=self.params[main_y_axis], tag=main_y_axis
            )
        ...
        dpg.add_drag_line(
            ...
            default_value=dpg.get_value("water_left_value"),
            parent="main_plot",
        )

        center = (
            dpg.get_value("water_left_value") + dpg.get_value("water_right_value")
        ) / 2

        dpg.add_drag_line(
            label="Water Center Line",
            tag="water_center_line",
        ...
        dpg.add_drag_line(
            ...
            tag="water_drag_right",
            show=False,
            color=(255, 255, 255, 255),
            callback=set_water_range_callback,
            user_data=(self.ui, "right"),
            default_value=dpg.get_value("water_right_value"),
            parent="main_plot",
        )
