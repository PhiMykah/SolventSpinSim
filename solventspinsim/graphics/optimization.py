...
class OptimizationSettings(Graphic):
    ...
    def render(self) -> None:
        ...
        with dpg.value_registry():
            dpg.add_float_value(default_value=self.params[water_left], tag=f"{water_left}_value")
            dpg.add_float_value(default_value=self.params[water_right], tag=f"{water_right}_value")
        ...
        with dpg.table(header_row=False, parent=self.parent):
            ...
            with dpg.table_row():
                dpg.add_drag_float(label='Water Left Limit', format="%.02f", source=f'{water_left}_value',
                                tag=water_left, callback=set_water_range_callback, user_data=(self.ui, 'left'))
                help_msg("Leftmost region of the water signal peak")

                dpg.add_drag_float(label='Water Right Limit', format="%.02f", source=f'{water_right}_value',
